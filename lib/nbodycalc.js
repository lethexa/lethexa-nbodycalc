...
module.exports.GravitySimulator = function() {
	...
	var calcAccelForObject = function( body, G, R ) {
		...
		bodies.forEach(function(otherBody) {
			if(otherBody === body)
				return;
			var position = otherBody.getPosition();
			accel += calcAccelFromPos(G, otherBody.getMass(), position, R);
		});
		return accel;
	};
	...
	this.updateTime = function(dt) {
		bodies.forEach(function(otherBody) {
			...
			var R = otherBody.getPosition();
			var V = otherBody.getVelocity();
			var A = calcAccelForObject(tOtherHandle, GRAVITY_CONSTANT, R);

			var KaV = V;
			var KaA = A;

			var KbR = R + 0.5 * tTimeDiff * KaV;
			var KbV = V + 0.5 * tTimeDiff * KaA;
			var KbA = calcAccelForObject(tOtherHandle, GRAVITY_CONSTANT, KbR);

			var KcR = R + 0.5 * tTimeDiff * KbV;
			var KcV = V + 0.5 * tTimeDiff * KbA;
			var KcA = calcAccelForObject(tOtherHandle, GRAVITY_CONSTANT, KcR);

			var KdR = R + tTimeDiff * KcV;
			var KdV = V + tTimeDiff * KcA;
			var KdA = calcAccelForObject(tOtherHandle, GRAVITY_CONSTANT, KdR);


			var vfV = ( (KaV + 2.0 * KbV + 2.0 * KcV + KdV) / 6.0 );
			var vfA = ( (KaA + 2.0 * KbA + 2.0 * KcA + KdA) / 6.0 );

			R += dt * vfV;
			V += dt * vfA;

			otherBody.setPosition(R);
			otherBody.setVelocity(V);
		});
	};
};
